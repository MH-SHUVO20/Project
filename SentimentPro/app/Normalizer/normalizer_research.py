...
class ResearchBanglaEcomNormalizer:
    ...
    def normalize_token(self, token: str) -> RuleAction:
        ...
        if self.looks_like_banglish(token):
            try:
                from phoneticbn import bn
                bengali_word = bn(lower)
                if bengali_word and bengali_word != lower:
                    return RuleAction("R15_PHONETIC", token, bengali_word, "phonetic_transliteration", 0.90)
            except Exception:
                pass
